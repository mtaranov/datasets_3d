...
output_dir=${PROJDIR}'/by_chr_dist_matched_'${contacts}
mkdir -p $output_dir

#for thres in 10 0; do
for thres in 10; do
run_file=${PROJDIR}'/run_get_by_chr_data_labels-w_indx_'$day'_'${contacts}'_'$thres'.sh'

echo "/users/mtaranov/local/anaconda2/bin/python $PROJDIR/get_by_chr_data_labels-w_indx.py $InteractionsFileCaptureC_zipped $pe_hindIII_file $output_dir $features_path $day $thres" > $run_file

chmod 777 $run_file

qsub -o $PROJDIR/o.run_get_by_chr_data_labels-w_indx_$day'_'${contacts}'_'$thres -e $PROJDIR/e.run_get_by_chr_data_labels-w_indx_$day'_'${contacts}'_'$thres $run_file
